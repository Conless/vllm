...
class NanoInferEngine:
    ...
    def __init__(self, graph_module: torch.fx.GraphModule,
        ...
        self.config = config
        self.input_buffers = [{}
                              ] * config.scheduler_config.max_num_nano_batches
    ...
    def dryrun(
        ...
        num_nano_batches = (self.config.scheduler_config.max_num_nano_batches
                            if use_fake_nano_batches else 1)
        ...
        for batch_idx in range(num_nano_batches):
            while node_queue[batch_idx]:
                ...
                if node.op == "placeholder":
                    ...
                    if isinstance(example_value, torch.Tensor):
                        if isinstance(example_value.shape[0], torch.SymInt):
                            if split_config.use_cudagraph:
                                ...
                                if (self.input_buffers[batch_idx].get(
                                        placeholder_idx) is None):
                                    self.input_buffers[batch_idx][
                                        placeholder_idx] = torch.zeros(
                                            (16384, ) +
                                ...
                                self.input_buffers[batch_idx][
                                    placeholder_idx][:num_tokens].copy_(
                                        args[placeholder_idx][:num_tokens])
                                env[batch_idx][node] = self.input_buffers[
                                    batch_idx][placeholder_idx][:num_tokens]
                            else:
                                env[batch_idx][node] = args[
                    ...
                    node_args = [
                        env[batch_idx][arg]
                        if isinstance(arg, torch.fx.Node) else arg
                        for arg in node.args
                    ]
                    node_kwargs = {
                        k: env[batch_idx][v]
                        if isinstance(v, torch.fx.Node) else v
                    ...
                    with (
                        (hook((op_handle, ))
                         if hook is not None else contextlib.nullcontext()),
                            torch.profiler.record_function(
                                f"op_{op_handle.module_name}_{op_handle.nano_batch_idx}"
                            ),
                            set_forward_context(
                                NanoInferContext(
                        ...
                        env[batch_idx][node] = module(*node_args,
                                                      **node_kwargs)
                elif node.op == "output":
    ...
    def execute_sequential(
        ...
        """Execute the model sequentially for a single nano-batch.

        This is an optimized path for when num_nano_batches == 1, avoiding
        the async producer-consumer pattern and scheduler coordination
        overhead.
        ...
        Args:
            ...
            split_config: Configuration for nano-batch splitting
                (must have num_nano_batches == 1)
            hook: Optional hook for operator execution

        Returns:
            Dictionary mapping nano-batch index to results and list of
            CUDA events
        """
        assert split_config.num_nano_batches == 1, \
            "execute_sequential only supports single nano-batch"
        assert not split_config.is_dryrun, \
            "execute_sequential is for runtime execution, use dryrun"

        batch_idx = 0
        env: dict[int, dict[torch.fx.Node, Any]] = {batch_idx: {}}
        results: dict[int, Any] = {}
        last_events = [torch.cuda.Event()]
        num_tokens_padded = split_config.num_tokens_padded[0]
        ...
        for node in self.graph_module.graph.nodes:
            if node.op == "placeholder":
                ...
                if isinstance(node.args[0], torch.fx.Node):
                    ...
                    results[batch_idx] = tuple(
                        env[batch_idx][arg] if isinstance(arg, torch.fx.Node
                                                          ) else arg
                        for arg in node.args[0])
                else:
                    results[batch_idx] = node.args[0]
                ...
                with (
                    (hook(operators)
                     if hook is not None else contextlib.nullcontext()),
                        set_forward_context(
                            NanoInferContext(
                                nano_batch_idx=tuple(op.nano_batch_idx
                                                     for op in operators),
                ...
                for batch_idx in range(num_nano_batches):
                    ...
                    while node_queue[batch_idx]:
                        ...
                        if node.op == "call_module":
                            ...
                            if isinstance(node.args[0], torch.fx.Node):
                                results[batch_idx] = env[batch_idx][
                                    node.args[0]]
                            elif isinstance(node.args[0], (tuple, list)):
                                results[batch_idx] = tuple(
                                    env[batch_idx][arg] if isinstance(
                ...
                if isinstance(example_value.shape[0], torch.SymInt):
                    if split_config.use_cudagraph:
                        ...
                        env[batch_idx][node] = self.input_buffers[batch_idx][
                            placeholder_idx][:num_tokens_padded]
                    else:
                        env[batch_idx][node] = args[placeholder_idx][
